...
PATH_SEGMENTS_SKIP = frozenset(
    {"app", "src", "lib", "services", "routes", "api", "handlers", "models"},
)
GENERIC_MODULE_STEMS = frozenset(
    {"utils", "common", "helpers", "base", "core", "config", "main", "index"},
)
...
def suggest_feature(sym: Symbol) -> str | None:
    """Guess a @feature grouping from module path (engineer confirms at ratify)."""
    rel = Path(sym.rel_path)
    stem = rel.stem
    if stem != "__init__" and stem not in GENERIC_MODULE_STEMS:
        return stem
    parent = rel.parent.name
    if parent not in PATH_SEGMENTS_SKIP:
        return parent
    return None


def format_feature_line(sym: Symbol) -> str:
    suggested = suggest_feature(sym)
    if suggested:
        return f"@feature {suggested}  # suggested — edit, confirm, or delete at ratify"
    return "@feature <name>  # optional grouping — engineer fills at ratify"
...
def render_queue(
    ...
    lines = [
        ...
        "",
        "Approve, edit, or reject each draft. Approved → paste into the docstring, flip",
        "`@provenance status` to `verified`, and confirm or edit `@feature` (optional grouping",
        "for wiki Features / Gherkin — infer suggests from module path).",
        "",
    ]
    if not missing:
