...
def _node_context(node: dict) -> str:
    return node.get("context") or "root"
...
def mermaid_map(entities, flags, edges) -> str:
    ...
    by_ctx: dict[str, dict[str, list[dict]]] = {}
    for n in entities:
        ctx = _node_context(n)
        by_ctx.setdefault(ctx, {}).setdefault(n["subsystem"], []).append(n)
    for ctx in sorted(by_ctx):
        ctx_label = page_name(ctx) if ctx != "root" else "root"
        out.append(f"  subgraph {ctx_label}")
        for sub, ns in sorted(by_ctx[ctx].items()):
...
def render(brain: Path, root: Path, nodes, edges, sha, stamp) -> dict:
    ...
    # Group by context first, then subsystem.  Single-context repos still use
    # the same module pages, just without a "Contexts" header.
    by_context: dict[str, dict[str, list[dict]]] = {}
    for n in entities:
        by_context.setdefault(_node_context(n), {}).setdefault(n["subsystem"], []).append(n)
    subsystems: dict[str, list[dict]] = {}
    for ctx in by_context:
        subsystems.update(by_context[ctx])
    ...
    # Adapter / boundary nodes: entities whose own context differs from the
    # context of the entity they connect to (cross-context edges).
    boundary_edges: list[tuple[str, str, str]] = []
    for e in edges:
        if e["type"] in ("gated-by", "raises"):
            # flags/exception nodes have no context and are shared infrastructure
            continue
        if e["type"] == "adapts":
            boundary_edges.append((e["from"], "adapts", e["to"]))
            continue
        # General cross-context entity-to-entity edge.
        fnode = ent_by_id.get(e["from"])
        tnode = ent_by_id.get(e["to"])
        if fnode and tnode and _node_context(fnode) != _node_context(tnode):
            boundary_edges.append((e["from"], e["type"], e["to"]))
    contexts = sorted(set(_node_context(n) for n in entities))
    multi_context = len(contexts) > 1 or (len(contexts) == 1 and contexts[0] != "root")
    ...
    if multi_context:
        ag += ["## Contexts", ""]
        for ctx in contexts:
            c_ents = by_context.get(ctx, {})
            c_count = sum(len(v) for v in c_ents.values())
            ag.append(f"- `{ctx}` — {c_count} entities")
        ag.append("")
    ag += ["## Functions", ""]
    for ctx in contexts:
        ctx_subs = by_context.get(ctx, {})
        if multi_context:
            ag.append(f"### context: {ctx}")
        for sub in sorted(ctx_subs):
            ag.append(f"#### module: {page_name(sub)}")
            for n in sorted(ctx_subs[sub], key=lambda x: x["title"]):
                f = n["facts"]
    ...
    if multi_context:
        ag += ["", "## Boundaries", "",
               "Cross-context adapter edges. These are the seams between bounded contexts.", ""]
        if boundary_edges:
            for src, etype, dst in sorted(boundary_edges):
                ag.append(f"- `{src}` —{etype}→ `{dst}`")
        else:
            ag.append("_No cross-context edges detected._")
    if house_rules:
        ag += ["", "## House rules (premises, must respect)", ""]
        for d in sorted(house_rules, key=lambda x: x["id"]):
            df = d["facts"]
            applies = f" — applies to: {df['applies_to']}" if df.get("applies_to") else ""
            ag.append(f"- `{d['id']}` [{df.get('enforcement') or 'advisory'}] — {d['title']}{applies}")
            if d.get("intent"):
    ...
    hlines = ["# knowledge wiki", "",
              ...
              "## Map", "", mermaid_map(entities, flags, edges), ""]
    if multi_context:
        hlines += ["## Contexts", "", "Each context is a bounded vocabulary (glossary + contracts). "
                   "Module pages are grouped by context; adapter nodes live in [Boundaries](Boundaries).", ""]
        for ctx in contexts:
            c_subs = by_context.get(ctx, {})
            c_count = sum(len(v) for v in c_subs.values())
            hlines.append(f"- `{ctx}` — {c_count} entities")
        hlines += ["", "## Boundaries", "",
                   "Cross-context adapter seams (auto-generated from cross-context edges).", ""]
        if boundary_edges:
            for src, etype, dst in sorted(boundary_edges):
                hlines.append(f"- `{src}` —{etype}→ `{dst}`")
        else:
            hlines.append("_No cross-context edges detected._")
        hlines.append("")
    hlines += ["## Modules", ""]
    for ctx in contexts:
        ctx_subs = by_context.get(ctx, {})
        if multi_context:
            hlines.append(f"### {ctx}")
        for sub in sorted(ctx_subs):
            hlines.append(f"- [{page_name(sub)}]({page_name(sub)}) — {len(ctx_subs[sub])} functions")
        if multi_context:
            hlines.append("")
    ...
    if multi_context:
        hlines.append("- [Boundaries](Boundaries) — cross-context adapter seams")
        hlines.append("")
    (brain / "Home.md").write_text("\n".join(hlines) + foot)

    if multi_context:
        blines = ["# Boundaries", "",
                  "Cross-context adapter seams: edges that connect different bounded contexts.", ""]
        if boundary_edges:
            for src, etype, dst in sorted(boundary_edges):
                blines.append(f"- `{src}` —{etype}→ `{dst}`")
        else:
            blines.append("_No cross-context edges detected._")
        (brain / "Boundaries.md").write_text("\n".join(blines) + foot)
    ...
    for ctx in contexts:
        ...
        if multi_context:
            side.append(f"- {ctx}")
        for sub in sorted(ctx_subs):
            side.append(f"  - [{page_name(sub)}]({page_name(sub)})")
    side += ["", "**Operations**", "- [Features](Features)", "- [Decisions](Decisions)"]
    if resources:
        side.append("- [Infrastructure](Infrastructure)")
    side += ["- [Changelog](Changelog)", "- [Agent context](agent-context)", ""]
    if multi_context:
        side += ["**Boundaries**", "- [Boundaries](Boundaries)", ""]
    side += ["**Runbooks**"]
